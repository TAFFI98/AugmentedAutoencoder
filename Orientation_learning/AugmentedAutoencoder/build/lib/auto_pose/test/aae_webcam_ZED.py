...
with tf.Session(config=config) as sess:
    ...
    while videoStream.isActive():
        ...
        img = cv2.resize(left_right_image[0],(128,128))
        print(img.shape)
        ...
        cv2.namedWindow('resized webcam input', cv2.WINDOW_NORMAL)    # Create window with freedom of dimensions
        cv2.resizeWindow('resized webcam input', 300, 700)
        cv2.namedWindow('pred view rendered', cv2.WINDOW_NORMAL)    # Create window with freedom of dimensions
        cv2.resizeWindow('pred view rendered', 300, 700)
